...
describe("minBy", () => {
  ...
  test("two elements", () => {
    ...
    expect(minBy([{ data: { score: 20 } }, { data: { score: 10 } }], (o) => o.data.score)).toEqual({
      data: { score: 10 },
    });
  });
  ...
  test("strings comparison", () => {
    expect(minBy(["apricot", "pear", "apple", "banana"], (fruit) => fruit)).toEqual("apple");
    expect(minBy(["apple", "pear", "banana"], (fruit) => fruit.length)).toEqual("pear");
  });
  ...
  test("first minimum occurrence", () => {
    expect(minBy([{ n: 1, m: 3 }, { n: 0, m: 2 }, { n: 2 }, { n: 0 }], (o: any) => o.m)).toEqual({ n: 0, m: 2 });
  });
